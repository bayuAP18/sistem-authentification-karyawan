...
app.post('/auth/register/begin', async (req, res) => {
  ...
  if (user.passkey_reset) {
    return res.status(403).json({ error: 'Passkey perlu di-reset oleh administrator terlebih dahulu' });
  }

  const existingPasskeys = await Passkey.findAll({ where: { user_id: user.id } });
...
app.post('/auth/register/complete', async (req, res) => {
  ...
  if (!expectedChallenge || !userId) {
    return res.status(400).json({ error: 'Sesi tidak valid, mulai ulang registrasi' });
  }
  ...
  await Passkey.create({
    ...
    credential_public_key: Buffer.from(registrationInfo.credentialPublicKey).toString('base64'),
  ...
  const user = await User.findByPk(userId);
  res.json({ ok: true, message: `Fingerprint berhasil didaftarkan untuk ${user.username}!` });
});
...
app.post('/auth/login/begin', async (req, res) => {
  ...
  if (passkeys.length === 0) {
    return res.status(400).json({ error: 'Belum ada fingerprint terdaftar untuk user ini' });
  }
...
app.post('/auth/login/complete', async (req, res) => {
  ...
  if (!expectedChallenge || !userId) {
    return res.status(400).json({ error: 'Sesi tidak valid, mulai ulang login' });
  }

  const credentialId = body.id;
  const passkey = await Passkey.findOne({ where: { credential_id: credentialId } });
  if (!passkey) {
...
function requireAdmin(req, res, next) {
  if (!req.session.isAdmin) return res.status(401).json({ error: 'Akses ditolak. Login sebagai admin.' });
  next();
}
...
app.post('/api/admin/employees', requireAdmin, async (req, res) => {
  ...
  const user = await User.create({ username, name: name || username, role: 'employee' });
...
app.delete('/api/admin/passkeys/:passkeyId', requireAdmin, async (req, res) => {
  ...
  if (!passkey) return res.status(404).json({ error: 'Passkey tidak ditemukan' });
...
app.post('/api/admin/employees/:id/reset-passkey', requireAdmin, async (req, res) => {
  const { id } = req.params;
  const user = await User.findByPk(id);
  if (!user) return res.status(404).json({ error: 'User tidak ditemukan' });
  ...
  res.json({ ok: true, message: 'Passkey berhasil di-reset. Karyawan dapat daftar ulang.' });
});
...
app.delete('/api/admin/employees/:id', requireAdmin, async (req, res) => {
  ...
  const user = await User.findByPk(id);
  if (!user) return res.status(404).json({ error: 'User tidak ditemukan' });
  if (user.role === 'admin') return res.status(403).json({ error: 'Tidak bisa hapus admin' });
...
app.post('/auth/identify', async (req, res) => {
  const { username } = req.body;
  if (!username) return res.status(400).json({ error: 'Nomor karyawan wajib diisi' });

  const user = await User.findOne({ where: { username } });
  if (!user) return res.status(404).json({ error: 'Nomor karyawan tidak ditemukan. Hubungi administrator.' });
...
app.get('/attendance/today', async (req, res) => {
  ...
  const records = await Attendance.findAll({ where: { user_id: userId, date: today } });
...
app.post('/attendance/checkin', async (req, res) => {
  ...
  const today = new Date().toISOString().slice(0, 10);
  const alreadyIn = await Attendance.findOne({ where: { user_id: userId, date: today, type: 'checkin' } });
  if (alreadyIn) return res.status(409).json({ error: 'Anda sudah check-in hari ini', record: alreadyIn });
  ...
  const user = await User.findByPk(userId, { attributes: ['username', 'name'] });
  broadcast({
...
app.post('/attendance/checkout', async (req, res) => {
  ...
  const today = new Date().toISOString().slice(0, 10);
  const checkIn = await Attendance.findOne({ where: { user_id: userId, date: today, type: 'checkin' } });
  if (!checkIn) return res.status(400).json({ error: 'Anda belum check-in hari ini' });

  const alreadyOut = await Attendance.findOne({ where: { user_id: userId, date: today, type: 'checkout' } });
  if (alreadyOut) return res.status(409).json({ error: 'Anda sudah check-out hari ini', record: alreadyOut });
...
sequelize
  ...
  .catch(err => {
    ...
    console.error('   Pastikan MySQL berjalan dan database "webauth" sudah dibuat.');
    process.exit(1);
  });
'''

pathlib.Path(r'C:\Users\bagus\Documents\webauth\server.js').write_text(SERVER, encoding='utf-8')
print('server.js OK')
